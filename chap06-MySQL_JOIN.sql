...
-- ON을 활용한 JOIN
-- 컬럼명이 같거나 다를 경우 on으로 서로 연관있는 컬럼에대한 조건을 작성하여 join
select
	a.menu_name,
    b.category_name
from
	tbl_menu a
-- inner join tbl_category on b.category_code = b.category_code;
join tbl category b on a.category_code = b.category_code;
...
-- 3.RIGHT JOIN
-- 오른쪽 테이블이 모든 레코드와 왼쪽 테이블에서 일치하는 레코드를 반환하는 SQL JOIN유형
select
	a.menu_name,
    b.category_name
from
...
-- 인덱스가 잘 구성되어 있는 작은 테이블에 유리하며, 전체 행을 스캔할 필요가 없을때 효율적이다.
...
select /* + HASH_JOIN(a) */		-- 주석처럼 보이지만 실제로 SQL힌트절로 작동한다.
	a.menu_name,				-- 'a'라는 별칭을 가진 테이블에 대해 해시조인 사용을 지시.
    b.category_name
from
	tbl_menu a
join tbl_category b on a.category_code = b.category_code;